...
```mermaid
flowchart TD
    A["Khách hàng nhắn tin / comment"] --> B{"Tin nhắn đến từ kênh nào?"}

    B --> B1["Facebook Page / Messenger"]
    B --> B2["Zalo OA / Zalo cá nhân"]
    B --> B3["Shopee Chat"]
    B --> B4["TikTok Shop / Livestream"]
    B --> B5["Instagram DM / Comment"]

    B1 --> C["Nhân viên / chủ shop mở từng app để kiểm tra"]
    B2 --> C
    B3 --> C
    B4 --> C
    B5 --> C

    C --> D["Đọc từng tin nhắn thủ công"]
    D --> E["Tự xác định khách hỏi gì: giá, size, tồn kho, phí ship, đổi trả, tình trạng đơn"]

    E --> F["Kiểm tra thông tin rải rác"]
    F --> F1["Bảng giá / file sản phẩm"]
    F --> F2["Tồn kho"]
    F --> F3["Chính sách đổi trả / bảo hành"]
    F --> F4["Thông tin đơn hàng cũ"]
    F1 --> G
    F2 --> G
    F3 --> G
    F4 --> G

    G["Soạn câu trả lời thủ công hoặc copy mẫu có sẵn"] --> H["Gửi phản hồi cho khách"]

    H --> I{"Khách có tín hiệu muốn mua không?"}

    I -->|Có| J["Thu thập thông tin chốt đơn: sản phẩm, size/màu, số điện thoại, địa chỉ"]
    J --> K["Bàn giao sang bước tạo đơn / đóng gói / vận chuyển"]
    K --> K1["Nhân viên vận hành / chủ shop xử lý đơn"]

    I -->|Chưa rõ / đang cân nhắc| L["Nhân viên tự nhớ hoặc ghi chú khách cần follow-up"]
    L --> M["Có thể quên follow-up nếu inbox tiếp tục tăng"]

    I -->|Không mua / hỏi chung| N["Không gắn nhãn rõ ràng hoặc để trôi hội thoại"]

    C --> O["Cuối ngày kiểm tra lại các app"]
    O --> P["Tự rà xem còn tin nào chưa trả lời / khách nào bị sót"]
    P --> Q["Báo cáo thủ công cho chủ shop nếu có"]
    ...
    classDef bottleneck fill:#ffe6e6,stroke:#cc0000,stroke-width:2px,color:#000;
```
...
```mermaid
flowchart TD
    A["Khách hàng nhắn tin / comment"] --> B["Inbox đa kênh"]

    B --> B1["Facebook / Messenger"]
    B --> B2["Zalo"]
    B --> B3["Shopee Chat"]
    B --> B4["TikTok Shop / Livestream"]
    B --> B5["Instagram"]

    B1 --> C["Auto-sync về Unified Inbox"]
    B2 --> C
    B3 --> C
    B4 --> C
    B5 --> C

    C --> D["Rule xử lý bước đầu"]
    D --> D1["Loại spam / tin không liên quan"]
    D --> D2["Gắn kênh nguồn"]
    D --> D3["Nhận diện khách cũ bằng user ID / số điện thoại / mã đơn"]
    D --> D4["Kiểm tra dữ liệu có sẵn: sản phẩm, tồn kho, chính sách, đơn hàng"]
    ...
    E["AI phân loại intent khách hàng"] --> E1["Hỏi giá / size / màu"]
    E --> E2["Hỏi tồn kho / phí ship"]
    E --> E3["Muốn mua / chốt đơn"]
    E --> E4["Khiếu nại / đổi trả"]
    E --> E5["Khách cũ cần chăm sóc lại"]
    E --> E6["Tin mơ hồ cần người kiểm tra"]

    E1 --> F["AI lead scoring"]
    E2 --> F
    E3 --> F
    E5 --> F
    ...
    G -->|Khách nóng| H["Đẩy lên hàng ưu tiên cho nhân viên xử lý trước"]
    G -->|Khách ấm| I["AI gợi ý câu trả lời + đặt follow-up"]
    G -->|Khách lạnh| J["Trả lời bằng template / FAQ có kiểm soát"]

    E4 --> K["Escalate cho người thật"]
    E6 --> K

    H --> L["AI draft câu trả lời theo context shop"]
    I --> L
    J --> L

    L --> M["Nhân viên / chủ shop review câu trả lời"]

    M --> N{"Duyệt được không?"}

    N -->|Có| O["Gửi phản hồi cho khách"]
    N -->|Cần sửa| P["Nhân viên chỉnh sửa rồi gửi"]
    N -->|AI sai / thiếu dữ liệu| Q["Fallback: nhân viên tự xử lý thủ công"]

    O --> R{"Khách muốn mua không?"}
    P --> R
    Q --> R

    R -->|Có| S["Auto tạo draft đơn hàng"]
    S --> S1["Sản phẩm"]
    S --> S2["Size / màu"]
    S --> S3["Số điện thoại"]
    S --> S4["Địa chỉ"]
    S --> T["Nhân viên xác nhận đơn"]
    T --> U["Bàn giao vận hành / đóng gói / giao hàng"]

    R -->|Chưa chốt| V["AI tạo reminder follow-up"]
    V --> W["Nhân viên nhận danh sách khách cần nhắc lại"]

    R -->|Không mua| X["Gắn nhãn hội thoại để phân tích sau"]

    O --> Y["Dashboard cuối ngày"]
    P --> Y
    Q --> Y
    U --> Y
    W --> Y
    ...
    Y --> Y1["Số tin đã xử lý"]
    Y --> Y2["Số khách nóng"]
    Y --> Y3["Số khách chưa được follow-up"]
    Y --> Y4["Số case AI cần người sửa"]
    Y --> Y5["Thời gian phản hồi trung bình"]
    ...
    classDef automation fill:#e6f4ff,stroke:#0077cc,stroke-width:2px,color:#000;
    classDef ai fill:#e8f8e8,stroke:#1a8f3a,stroke-width:2px,color:#000;
    classDef human fill:#fff4d6,stroke:#d68a00,stroke-width:2px,color:#000;
    classDef fallback fill:#ffe6e6,stroke:#cc0000,stroke-width:2px,color:#000;
    classDef dashboard fill:#f0e6ff,stroke:#7a3fd1,stroke-width:2px,color:#000;
```
